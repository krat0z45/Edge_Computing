...
import { Thermometer, Snowflake, DoorOpen } from 'lucide-react';
...
export function HouseMockup({ isACOn, motionDetected }: HouseMockupProps) {
  return (
    <div className="relative w-full max-w-sm mx-auto aspect-square rounded-lg bg-muted/30 p-4 border-2 border-dashed border-border flex items-center justify-center">
      <div className={cn(
        ...
        {/* Living Room */}
        <div className={cn(
          "absolute top-4 left-4 bottom-4 w-1/2 bg-card/50 rounded-l-sm transition-colors duration-200",
        ...
        {/* Bedroom */}
        <div className="absolute top-4 right-4 bottom-4 w-[calc(50%-2rem)] bg-card/50 rounded-r-sm">
           <div className="p-2 text-center">
        ...
        <div className={cn(
           ...
           <Snowflake className={cn("h-6 w-6", {"animate-spin-slow": isACOn})} />
        </div>
      </div>
       <style jsx>{`
        .animate-spin-slow {
